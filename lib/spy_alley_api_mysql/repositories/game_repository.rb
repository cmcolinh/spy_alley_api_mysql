  ...
  module Repositories
    class GameRepository < ROM::Repository[:games]
      ...
      def create_game_and_get_game_id
        ...
        create_game.({})
        games.select{integer::last_insert_id.as(:id)}.first[:id].to_i
      end
      ...
      def create_players(player_tuples, game_id)
        ...
        games.join(:players)
          .select(players[:id], players[:seat_id])
      ...
      def create_initial_action(new_players, game_id)
        ...
        actions.select{integer::last_insert_id.as(:id)}.first[:id].to_i
      end

      def create_spy_identities(new_players, action_id)
        create_spy_identities = spy_identities.command(:create)
        spy_identities_to_use = [1, 2, 3, 4, 5, 6].sample(new_players.size)
        spy_identity_tuples = new_players.map.with_index do |new_player, index|
          {
      ...
      def create_player_resources(new_players, action_id)
        create_player_resources = player_resources.command(:create)
